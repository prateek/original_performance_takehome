...
- Static schedule snapshot (current `1378`-cycle kernel): load uses `2` slots in `1308` cycles, `1` slot in `1` cycle, and `0` slots in `69` cycles (largest load-idle runs: cycles `31–66` and `1367–1377`). Any `< 1363` change likely needs eliminating ~`15` load-idle cycles (better overlap) or reducing total load slots.
- Quick sweep: brute-forcing the simple piecewise start-stagger knobs (`early_groups`, `early_spacing`, `late_spacing`) didn’t beat `1378` (best remains `4/4/16`), so further improvements likely need richer schedule logic changes (not just spacing tweaks).
