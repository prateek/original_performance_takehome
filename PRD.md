...
- Current best is **`1365` cycles** on the authoritative harness (`python tests/submission_tests.py`).
- The remaining gap to the next submission threshold (`< 1363`, i.e. `1362` or better) is **3 cycles**.
...
- Static schedule snapshot (current `1365`-cycle kernel): load uses `2` slots in `1308` cycles, `1` slot in `1` cycle, and `0` slots in `56` cycles (largest load-idle runs: cycles `29–62` and `1354–1364`). Any `< 1363` change likely needs eliminating ~`3` load-idle cycles (better overlap) or reducing total load slots.
