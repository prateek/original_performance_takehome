...
- Current best is **`1356` cycles** on the authoritative harness (`python tests/submission_tests.py`).
...
- Static schedule snapshot (previous `1365`-cycle kernel; current best `1356`): load uses `2` slots in `1302` cycles and `0` slots in `54` cycles (largest load-idle runs: cycles `22–54` and `1345–1355`). Any further improvements likely need eliminating load-idle cycles (better overlap) or reducing total load slots.
