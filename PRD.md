...
- Static schedule analysis: `python analyze_schedule.py` (prints per-engine utilization + idle runs).
- Debug/trace loop:
...
- Current best is **`1354` cycles** on the authoritative harness (`python tests/submission_tests.py`, revalidated 2026-01-30).
...
- Static schedule snapshot (current best `1354`): load uses `2` slots in `1301` cycles and `0` slots in `53` cycles (largest load-idle runs: cycles `21–53` and `1343–1353`). Any further improvements likely need eliminating load-idle cycles (better overlap) or reducing total load slots.
...
- 2026-01-30: Added `analyze_schedule.py` to summarize per-engine utilization and idle runs from the statically-built kernel schedule; verified authoritative submission cycles remain `1354`.
- 2026-01-30: Added `tune_start_offsets.py` to automate `START_OFFSETS` hill-climb sweeps; quick searches did not beat `1354` cycles (authoritative `python tests/submission_tests.py` remains `1354`).
- 2026-01-30: Re-ran authoritative submission tests (`python tests/submission_tests.py`) and did a small targeted sweep over `START_OFFSETS` (single-index deltas + tail-group grids); no improvement found (best remains `1354`).
- 2026-01-30: Ran a longer `START_OFFSETS` hill-climb (`python tune_start_offsets.py --iters 20000 --seed 2 --max-delta 32 --multi 2`); best remained `1354`. Re-ran authoritative submission tests; cycles remain `1354`.
- 2026-01-30: Tried offloading the depth-2 fast-path `idx=={4,5,6}` vector compares from `valu` to lane-wise scalar `alu` ops (to reduce `valu` pressure); it regressed to `1388` cycles, so it was reverted. Re-ran authoritative submission tests; cycles remain `1354`.
- 2026-01-30: Re-ran authoritative submission tests (`python tests/submission_tests.py`) to confirm the current kernel still scores `1354` cycles; updated PRD/progress log.
- 2026-01-30: VLIW packer: include latency-0 deps in the criticality metric (prioritize slots that unlock long chains within a cycle). Authoritative submission cycles unchanged at `1354` (`python tests/submission_tests.py`).
- 2026-01-30: Tried a depth-3 fast path (idx in `{7..14}`) to reduce gather pressure via preloading nodes `[8..15]` and doing a flow-engine `vselect` LUT (bits from ALU). It regressed to `1435` cycles, so it was reverted. Authoritative submission cycles remain `1354` (`python tests/submission_tests.py`).
- 2026-01-30: Main-loop scheduler: rotate `xor_queue` selection so POSTLOAD_XOR groups don’t head-of-line block the ALU XOR stage when some candidates aren’t ready yet. Authoritative submission cycles unchanged at `1354` (`python tests/submission_tests.py`).
- 2026-01-30: Tried opportunistically offloading depth 0–2 vector XORs to lane-wise scalar ALU XORs (when ALU budget allowed) to reduce `valu` pressure; static schedule regressed to `1397` cycles (`python analyze_schedule.py`), so it was reverted. Authoritative submission cycles remain `1354` (`python tests/submission_tests.py`).
