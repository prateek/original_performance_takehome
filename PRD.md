...
- Current best is **`1358` cycles** on the authoritative harness (`python tests/submission_tests.py`).
...
- Static schedule snapshot (previous `1365`-cycle kernel; current best `1358`): load uses `2` slots in `1302` cycles and `0` slots in `56` cycles (largest load-idle runs: cycles `22–54` and `1347–1357`). Any further improvements likely need eliminating load-idle cycles (better overlap) or reducing total load slots.
