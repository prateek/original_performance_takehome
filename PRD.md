...
- Current best is **`1359` cycles** on the authoritative harness (`python tests/submission_tests.py`).
...
- Static schedule snapshot (previous `1365`-cycle kernel; current best `1359`): load uses `2` slots in `1308` cycles, `1` slot in `1` cycle, and `0` slots in `56` cycles (largest load-idle runs: cycles `29–62` and `1354–1364`). Any further improvements likely need eliminating load-idle cycles (better overlap) or reducing total load slots.
