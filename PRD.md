...
- Current best is **`1368` cycles** on the authoritative harness (`python tests/submission_tests.py`).
- The remaining gap to the next submission threshold (`< 1363`) is **5 cycles**.
...
- Static schedule snapshot (current `1368`-cycle kernel): load uses `2` slots in `1308` cycles, `1` slot in `1` cycle, and `0` slots in `59` cycles (largest load-idle runs: cycles `31–64` and `1357–1367`). Any `< 1363` change likely needs eliminating ~`5` load-idle cycles (better overlap) or reducing total load slots.
